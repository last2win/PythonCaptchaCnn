# -*- coding: utf-8 -*-
"""
Created on Sun Jun  2 21:42:29 2019
...
from singleCaptchaGenerate import CAPTCHA_HEIGHT, CAPTCHA_WIDTH, CAPTCHA_LENGTH, CAPTCHA_LIST
...
def cnn_graph(x, keep_prob, size, captcha_list=CAPTCHA_LIST, captcha_len=CAPTCHA_LENGTH):
    """
    三层卷积神经网络
    :param x:   训练集 image x
...
def accuracy_graph(y, y_conv, width=len(CAPTCHA_LIST), height=CAPTCHA_LENGTH):
    """
    偏差计算图，正确值和预测值，计算准确度
    :param y: 正确值 标签
    :param y_conv:  预测值
...
def train(height=CAPTCHA_HEIGHT, width=CAPTCHA_WIDTH, y_size=len(CAPTCHA_LIST)*CAPTCHA_LENGTH):
    """
    cnn训练
    :param height: 验证码高度
    :param width:   验证码宽度
    ...
    step = 0    # 步数
    while 1:
        batch_x, batch_y = generateData(64)
        sess.run(optimizer, feed_dict={x: batch_x, y: batch_y, keep_prob: 0.75})
        # 每训练一百次测试一次
        if step % 100 == 0:
            batch_x_test, batch_y_test = generateData(100)
